...
def main():
    ...
    try:
        ...
        # Clean up thumbnails in cache generated by this run
        if 'thumbnail_map' in locals() and thumbnail_map:
            for thumb_path in thumbnail_map.values():
                try:
                    if thumb_path and os.path.exists(thumb_path):
                        os.remove(thumb_path)
                except Exception as e:
                    print(f"Warning: Could not remove thumbnail {thumb_path}: {e}", file=sys.stderr)
